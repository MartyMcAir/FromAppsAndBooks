...
//        document = new HTMLDocument();
    ...
    public void openDocument() { // 23 долж работ аналогичн saveDocumentAs(),в той части,котор отвеч за выбор файла.
        // openDocument() вызывает метод selectHtmlTab() у представления.
        view.selectHtmlTab();
        ...
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            resetDocument();
            // устанавливает имя файла в качестве заголовка окна представления, если пользователь подтвердит выбор файла.
            view.setTitle(currentFile.getName());
            // 23.2.3. Установить имя файла в заголовок у представления.
//            view.setTitle("Open File");  // а вот это не верно
            ...
            try {
                ...
                HTMLEditorKit htmlEditorKit = new HTMLEditorKit();
                // сбрасывает правки через метод resetUndo(), если пользователь подтвердит выбор файла.
                view.resetUndo();
    ...
    public void saveDocument() {
        // saveDocument() вызывает метод selectHtmlTab() у представления.
        view.selectHtmlTab();
        // 23.1 должен работать также, как saveDocumentAs(), но не запрашивать файл у пользов, а использ currentFile.
...
//            JOptionPane.showMessageDialog(view, "Сохранено"); // для уведомления и проверки, что сохранено
        }
    }
